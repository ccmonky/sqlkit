...
func conversionTests() []conversionTest {
	...
	return []conversionTest{
		...
		{s: "1.5", d: new(userDefined), wantusrdef: 1.5},
		{s: []byte{1, 2, 3}, d: new(userDefinedSlice), wanterr: `unsupported Scan, storing driver.Value type []uint8 into type *sqlkit_test.userDefinedSlice`},
...
func TestConversions(t *testing.T) {
	for n, ct := range conversionTests() {
		err := sqlkit.ConvertAssign(ct.d, ct.s)
		errstr := ""
		if err != nil {
			errstr = err.Error()
		}
...
func TestNullString(t *testing.T) {
	...
	sqlkit.ConvertAssign(&ns, []byte("foo"))
	if !ns.Valid {
...
func TestRawBytesAllocs(t *testing.T) {
	...
	test := func(name string, in any, want string) {
		if err := sqlkit.ConvertAssign(&buf, in); err != nil {
...
func TestDecimal(t *testing.T) {
	...
	for _, item := range list {
		t.Run(item.name, func(t *testing.T) {
			out := dec{}
			err := sqlkit.ConvertAssign(&out, item.in)
			if item.err {
				if err == nil {
